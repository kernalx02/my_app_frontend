...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // If user is already logged in, show them the feed instead of landing
      setPage('posts'); 
    }
    refreshPosts();
  }, []);

  // --- FIX: LISTEN FOR LOGIN EVENT AND REDIRECT ---
  useEffect(() => {
    const handleStorageChange = () => {
      const user = localStorage.getItem('currentUser');
      if (user) {
        setCurrentUser(JSON.parse(user));
        // FIX: This line ensures the view switches to the feed immediately upon login
        setPage('posts'); 
      } else {
        setCurrentUser(null);
        setPage('landing');
